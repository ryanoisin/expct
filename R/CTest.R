...
CTest = function(differentialtimevaryingpredictors = differentialtimevaryingpredictors,
  ...
  if (is.null(controlvariables)==TRUE){ #IF CONTROL VARIABLES ARE NOT SPECIFIED
   # OR edited - check?
    model<-as.formula(paste("outcome~",differentialtermlist,ifelse(k3 <3,"", paste("+s(time,k=",k3,")"))))
  ...
  for(i in 1:length(outcome)){
    for(ii in 1:length(differentialtimevaryingpredictors)){
      ...
      plot(mod,select=graphnumber,ylab=paste("Beta Coefficient of ",differentialtimevaryingpredictors[ii],"lag on ",outcome[i],sep=""),xlab="Time Differences",main="CTVEM Estimation stage")
      }
        }
  }
