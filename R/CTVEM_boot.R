...
CT_LAG_boot<-function(data=NULL,Time="Time",outcome=NULL,ID="ID", estimate = "marginal",iterations = 50, quantiles = c(.025, 0.975),boot = TRUE, Mean = TRUE, Tpred = seq(0,50,1), plot_show = FALSE, ncores = NULL, standardized=TRUE,method = "bam", gamma=1,k=10,k3=3){
  if(is.null(ncores)){
  ...
  bootstrap_results = foreach(iii=1:iterations, .combine="rbind",.options.snow = opts,.export=c("CT_LAG_single","datamanipulation","CTest"),.packages = c("plyr","zoo","reshape2","mgcv","matrixStats")) %dopar%{
    ...
    CT_LAG_single(data = data_select,Time = Time,ID = ID, estimate = estimate, Tpred = Tpred, plot_show = plot_show,outcome = outcome,boot = boot, standardized = standardized, method = method, gamma = gamma,k = k, k3 = k3)
  }
  stopCluster(cl)
  ...
  attributes = list("Outcome variables" = outcome, "Estimate type" = estimate, "Bootstrapping times" = iterations, "If standardize data" = standardized, "method" = method, "gamma" = gamma, "k" = k, "k3" = k3  )
